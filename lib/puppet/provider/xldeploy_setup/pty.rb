    ...
    PTY.spawn(command) do |input, output, pid|
      ...
      input.each { |line|
        ...
        if line =~ /Options are yes or no./
          Puppet.debug "Hitting options"
          output.puts('no') if line_array[-1] =~ /An existing XL Deploy Server configuration was found. Do you want to edit it/
          ...
          output.puts('no') if line_array[-1] =~ /Would you like to enable SSL/
        ...
        output.puts("dummyyyyyyyyyy") if line =~ /Please enter the admin password you wish to use for XL Deploy Server/
        ...
        output.puts("\r") if (line =~ /Please enter the password you wish to use./) 
